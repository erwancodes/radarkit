...
function SignalPage() {
  ...
  return <RootShell><article className="page-container signal-page">
    ...
    <div className="detail-body"><div className="detail-prose">{signal.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><div className="detail-source"><span>Source originale</span><a href={signal.sourceUrl} target="_blank" rel="noreferrer">Ouvrir l’article officiel <ArrowUpRight size={16} /></a><small>{signal.source} · RadarKit garde la provenance visible. Il ne remplace pas la source.</small></div></div>
  </article></RootShell>
}
